...
const { ensureCsrfCookie, attachUser, requireAuth, requireAdmin } = require("./auth");
...
// One counter, no per-visitor tracking (no IP logging, no cookies, no third
// party) — the client calls this once per page load. It's a rough "how many
// times has this page been opened" number, not unique visitors.
app.post("/api/track-visit", (req, res) => {
  statements.bumpCounter.run("total_page_loads");
  res.json({ ok: true });
});

// Set ADMIN_USERNAME in your environment to the account you log in with —
// only that account can see this. See README's Environment variables table.
app.get("/api/admin/stats", requireAuth, requireAdmin, (req, res) => {
  const activeRooms = Object.keys(rooms).length;
  const activeUsers = Object.values(rooms).reduce((sum, r) => sum + Object.keys(r.users).length, 0);
  res.json({
    totalAccounts: statements.getUserCount.get().count,
    totalPageLoads: (statements.getCounter.get("total_page_loads") || {}).value || 0,
    totalRoomsCreated: (statements.getCounter.get("total_rooms_created") || {}).value || 0,
    totalRoomJoins: (statements.getCounter.get("total_room_joins") || {}).value || 0,
    activeRoomsRightNow: activeRooms,
    activeUsersRightNow: activeUsers,
  });
});
...
app.post("/api/create-room", (req, res) => {
  ...
  statements.bumpCounter.run("total_rooms_created");
  res.json({ roomId });
});
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", ({ roomId, name, role, pin }) => {
    ...
    statements.bumpCounter.run("total_room_joins");
  ...
  // ---- Ambient soundscape choice ----
  // Just relays WHICH sound (or "off") — the actual audio is generated
  // locally in each browser via the Web Audio API, nothing streams here.
  socket.on("ambient", ({ sound }) => {
    if (!currentRoom) return;
    const ALLOWED = ["rain", "waves", "fireplace", "brown", null];
    if (!ALLOWED.includes(sound || null)) return;
    socket.to(currentRoom).emit("ambient", { sound: sound || null, from: currentName });
  });
