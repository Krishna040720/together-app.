...
- **Drift correction** — YouTube (including the Echo Music tab) and uploaded videos now quietly re-sync every few seconds in the background, not just when someone hits play/pause. This fixes the "starts together but slowly drifts out of sync" problem.
...
- **Android Chrome**: screen sharing works, but only for that one browser tab — switching to another app (like Instagram) freezes what the other person sees, because the tab goes into the background. The app now tells them plainly when this happens rather than leaving it a silent frozen frame.
- **Bottom line for now**: call + YouTube/Echo Music/uploaded-video watching works well cross-device (phone-to-laptop, phone-to-phone). Screen share is most reliable laptop-to-laptop for the moment.

## About the Echo Music tab

Spotify's embedded player was replaced with an **Echo Music** tab. Echo Music itself streams from YouTube Music's catalog rather than hosting its own tracks, so there's no separate embeddable player/API for it to plug in — instead, this tab is a themed front door onto the same reliable YouTube player and sync logic the YouTube tab already uses. You can search for a song or artist right in the tab (results biased toward music) and tap one to load it — no pasting required — or paste a `music.youtube.com` (or regular `youtube.com`) link directly if you prefer. Either way it loads and stays in sync the same way a YouTube video does — no login/Premium requirement, no "open in app" handoff breaking sync, no preview-only playback. Keyword search uses the same free YouTube Data API key as the YouTube tab's search (you'll be prompted for one the first time, and it's remembered after that).
