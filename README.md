...
- **Invite links**: creating a room gives you a "🔗 Copy invite link" button in the top bar — send that instead of a code, and it auto-fills the join screen. Room codes are also normalized (trimmed, lowercased) so a typo in casing/spacing can't accidentally create two different rooms.
- **Screen share**: click 🖥️ in the call controls (or the "Browse / Screen share" tab). This adds your screen as a *second* video track alongside your camera — so your camera bubble keeps working the whole time, your voice keeps coming through normally, and there's no re-encoding lag because it's a direct WebRTC track, not a canvas/screen-recording hack. Click again (or use the browser's native "Stop sharing" bar) to end it.
- **Spotify**: paste a track/album/playlist link under the Spotify tab. Uses Spotify's official embeddable player + its iFrame Control API, so play/pause/seek sync the same way YouTube does. Note: Spotify's embed player plays full tracks for everyone, but some catalog restrictions (regional availability, occasional 30-second previews for non-Premium accounts on certain content) are controlled by Spotify itself, not this app.
- **Clearer call tiles**: bigger rounded video tiles instead of tiny circles, a mic-muted badge that shows on the other person's tile when you mute, and a mirrored self-view (feels more natural, like a mirror rather than reversed handwriting).
