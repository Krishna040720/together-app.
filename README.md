...
**Live:** https://together-app-3.onrender.com/
...
None of these are required to run it locally — the server falls back to sane
defaults for zero-setup dev use. For a real deployment (Render/Railway/etc), set:

| Variable | Required? | What it does |
|---|---|---|
| `PORT` | No | Set automatically by Render/Railway — don't set it yourself. |
| `NODE_ENV=production` | **Recommended** | Makes auth cookies `Secure` (HTTPS-only). Without it, login still works over HTTPS, but the cookie doesn't get the extra `Secure` flag. |
| `JWT_SECRET` | **Recommended** | Signs login session tokens. Without it, a random secret is generated at startup — fine for a demo, but it means **every restart logs everyone out**, and if you ever run more than one server instance they won't recognize each other's sessions. |
| `CORS_ORIGIN` | No | Locks the API + Socket.IO down to one frontend origin (e.g. `https://your-app.netlify.app`). Without it, the server reflects whatever origin the request came from — fine for the common "server also serves the client" setup, worth setting explicitly if client and server are on different domains. |
| `TURN_USERNAME` / `TURN_CREDENTIAL` | No | Your own TURN credentials from [metered.ca/tools/openrelay](https://www.metered.ca/tools/openrelay/), for more reliable cross-network calls than the shared public fallback. |
| `ADMIN_USERNAME` | No | Set this to the username of your own account to unlock `/admin.html` — a simple usage-stats page (total accounts, total rooms created, total room joins, page loads, and what's active right now). Log in as that account in the app first, then visit `/admin.html` in the same browser. Without this set, `/api/admin/stats` just 404s for everyone, including you. |

**Node version:** `server/package.json` pins `"engines": { "node": ">=18 <26" }`. This isn't just a style preference — `better-sqlite3` (used for the optional-accounts DB) ships prebuilt binaries only for released Node versions; if Render/Railway ever defaults to a Node version newer than what's covered here, the install falls back to compiling it from source, which can fail on the build machine (missing build tools, ABI mismatch, etc). If your deploy fails with an `npm error ... prebuild-install ... No prebuilt binaries found` message, this range is why — either wait for `better-sqlite3` to publish binaries for the newer Node, or explicitly set a `NODE_VERSION` env var on your host to something in this range.
